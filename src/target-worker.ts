...
import { CliGitBackend } from "./git.js";
import { DockerImageAdapter } from "./deployment.js";
...
export async function startTargetWorker(options: TargetWorkerOptions) {
  ...
  const git = new CliGitBackend({ root: options.workspaceRoot });
  const gitAvailable = await git.available();
  const image = options.imageRepository && options.registry ? new DockerImageAdapter({ workspaceRoot: options.workspaceRoot, imageRepository: options.imageRepository, registry: options.registry }) : undefined;
  const capabilities = ["filesystem.read", "filesystem.write", "execution.run", ...(gitAvailable ? ["git.commit", "git.push"] : []), ...(image ? ["image.build", "image.push"] : [])] as import("./target.js").ExecutionTargetCapability[];
  const description: TargetDescription = { id: options.targetId, kind: "development-pc", available: true, capabilities, workspace: { id: options.workspaceId, path: options.workspaceRoot, repository: "git@github.com:eugen252009/Porta.git" }, platform: platform() };
