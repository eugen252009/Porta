...
export class DelegatedTaskApplicationService {
  ...
  async create(request: DelegatedTaskRequest, principal: Principal): Promise<DelegatedTaskAccepted> {
    ...
    const task = await this.context.delegations.createChild(`remote-parent:${principal.identity}`, permissions, toDelegationRequest(request), { delegationId: request.delegationId, parentNodeId: principal.identity, parentIdentity: principal.identity, childNodeId: this.nodeId, authority: "child" });
  ...
  async cancelForPrincipal(id: string, principal: Principal): Promise<DelegatedTask> {
    ...
    if (principal.kind === "node") return this.context.delegations.cancel(id);
