...
export const portaConfigSchema = z.object({
  ...
  git: z.object({ enabled: z.boolean().default(false), executable: z.string().min(1).optional(), maxDiffBytes: z.number().int().positive().default(262144), maxShowBytes: z.number().int().positive().default(262144), maxLogEntries: z.number().int().positive().max(100).default(20) }).optional(),
});
