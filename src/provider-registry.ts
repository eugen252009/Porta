...
import { providerLabel, withModelIdentity } from "./model-identity.js";
...
export class ProviderRegistry {
  ...
  async models(): Promise<readonly ModelOption[]> { const result: ModelOption[] = []; for (const provider of this.providers) for (const model of await this.probe(provider.type, provider.endpoint)) result.push(withModelIdentity(model, provider.id, providerLabel(provider.type), provider.name)); return result; }
