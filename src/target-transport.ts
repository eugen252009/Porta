...
export class HttpTargetTransport implements TargetTransport {
  ...
  async createApplicationSession(input: { readonly sessionId?: string; readonly target?: string; readonly model?: { readonly provider: string; readonly model: string } }): Promise<NodeSessionSnapshot> { return this.request("POST", "/node/application/sessions", input, undefined, true); }
  async listApplicationTasks(): Promise<readonly NodeTaskSummary[]> { return this.request("GET", "/node/application/tasks", undefined, undefined, true); }
  async listApplicationSessions(): Promise<readonly NodeSessionSnapshot[]> { return this.request("GET", "/node/application/sessions", undefined, undefined, true); }
  ...
  async submitApplicationSession(sessionId: string, input: string): Promise<readonly KernelEvent[]> { return this.request("POST", `/node/application/sessions/${encodeURIComponent(sessionId)}/messages`, { input }, undefined, true); }
...
async function handleTargetRequest(request: IncomingMessage, response: ServerResponse, options: TargetTransportServerOptions, login: LoginService): Promise<void> {
  try {
    ...
    if (options.application && request.method === "POST" && url.pathname === "/node/application/sessions") { const identity = authenticateRequest(login, request); if (!identity) return send(response, 401, { error: "TARGET_AUTHENTICATION_REQUIRED" }); return send(response, 200, await options.application.createSession(await readBody(request) as { sessionId?: string; target?: string; model?: { provider: string; model: string } }, identity)); }
    if (options.application && request.method === "GET" && url.pathname === "/node/application/tasks") { const identity = authenticateRequest(login, request); if (!identity) return send(response, 401, { error: "TARGET_AUTHENTICATION_REQUIRED" }); return send(response, 200, await options.application.listTasks(identity)); }
    if (options.application && request.method === "GET" && url.pathname === "/node/application/sessions") { const identity = authenticateRequest(login, request); if (!identity) return send(response, 401, { error: "TARGET_AUTHENTICATION_REQUIRED" }); return send(response, 200, await options.application.listSessions(identity)); }
    ...
    if (options.application && request.method === "POST" && url.pathname.startsWith("/node/application/sessions/") && url.pathname.endsWith("/messages")) { const identity = authenticateRequest(login, request); if (!identity) return send(response, 401, { error: "TARGET_AUTHENTICATION_REQUIRED" }); const sessionId = decodeURIComponent(url.pathname.slice("/node/application/sessions/".length, -"/messages".length)); const body = await readBody(request) as { input?: string }; if (typeof body.input !== "string" || !body.input.trim()) return send(response, 400, { error: "SESSION_INPUT_REQUIRED" }); return send(response, 200, await options.application.submitSession(sessionId, body.input, identity)); }
  ...
  } catch (error) { const message = error instanceof Error ? error.message : "TARGET_TRANSPORT_FAILED"; const status = /authorization|depth|loop|policy|forbidden|denied/i.test(message) ? 403 : /unsupported|not found|unavailable/i.test(message) ? 404 : 500; return send(response, status, { error: message }); }
}
