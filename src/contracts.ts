...
export interface ModelCapabilities { readonly tools?: boolean; readonly streaming?: boolean; readonly reasoning?: boolean }
export interface ModelOption { readonly id: string; readonly displayName: string; readonly provider: string; readonly ref?: string; readonly connectionId?: string; readonly providerDisplayName?: string; readonly connectionDisplayName?: string; readonly capabilities?: ModelCapabilities }
export interface ModelSelection { readonly provider: string; readonly model: string; readonly modelRef?: string; readonly connectionId?: string; readonly capabilities?: ModelCapabilities }
export interface ModelControlMessage { readonly role: "system"; readonly content: string }
export interface ModelRequest { schemaVersion: 1; requestId: string; input: string; messages?: readonly ModelMessage[]; control?: readonly ModelControlMessage[]; tools?: readonly ToolDescriptor[]; temperature?: number }
...
export type ApprovalCommandDecision = "approve" | "deny";
export interface ApprovalRequestedEvent { type: "ApprovalRequested"; approvalId: string; toolCallId: string; toolId: string; input: JsonValue; descriptor?: ToolDescriptor; executionId: string; sessionId: string; traceId: string }
...
export interface Session { schemaVersion: 1; id: string; state: "open" | "closed"; createdAt: string; target?: string; model?: ModelSelection }
...
export interface ExecutionRequest { schemaVersion: 1; executable?: string; language?: ExecutionLanguage; source?: string; args?: readonly string[]; stdin?: string; workingDirectory?: string; environment?: Record<string, string>; timeoutMs?: number; policy: ExecutionPolicy }
export const executionRequestSchema = z.object({ schemaVersion, executable: z.string().min(1).optional(), language: z.enum(["javascript", "typescript"]).optional(), source: z.string().optional(), args: z.array(z.string()).optional(), stdin: z.string().optional(), workingDirectory: z.string().optional(), environment: z.record(z.string()).optional(), timeoutMs: z.number().int().positive().optional(), policy: executionPolicySchema });
...
export interface ToolDescriptor { id: string; name: string; version: string; description?: string; inputSchema: JsonValue; metadata?: JsonValue; capabilities?: readonly string[] }
export const toolDescriptorSchema = z.object({ id: toolId, name: z.string().min(1), version: z.string().min(1), description: z.string().optional(), inputSchema: toolSchema, metadata: toolSchema.optional(), capabilities: z.array(z.string().min(1)).optional() });
...
export type KernelCommand = { type: "CreateSession"; sessionId?: string; target?: string; model?: ModelSelection } | { type: "SubmitInput"; sessionId: string; input: string; mode?: ExecutionMode } | { type: "CancelExecution"; sessionId: string } | { type: "CloseSession"; sessionId: string } | { type: "ResolveApproval"; approvalId: string; decision: ApprovalCommandDecision; reason?: string };
export const resolveApprovalCommandSchema = z.object({ type: z.literal("ResolveApproval"), approvalId: z.string().min(1), decision: z.enum(["approve", "deny"]), reason: z.string().min(1).optional() }).strict();
export type KernelEvent = { type: "ExecutionStarted"; executionId: string } | { type: "OutputStarted" } | { type: "OutputDelta"; text: string } | { type: "OutputCompleted" } | { type: "Warning"; message: string } | { type: "Error"; error: HarnessError } | { type: "ExecutionCompleted" } | { type: "ExecutionCancelled" } | { type: "SessionCreated"; sessionId: string } | { type: "SessionClosed"; sessionId: string } | { type: "ToolRequested"; toolCallId: string; toolId: string } | { type: "ToolStarted"; toolCallId: string; toolId: string } | { type: "ToolCompleted"; toolCallId: string; toolId: string; result: ModelToolResult } | ApprovalRequestedEvent | ApprovalResolvedEvent;
export const kernelEventSchema = z.discriminatedUnion("type", [z.object({ type: z.literal("ExecutionStarted"), executionId: id }), z.object({ type: z.literal("OutputStarted") }), z.object({ type: z.literal("OutputDelta"), text: z.string() }), z.object({ type: z.literal("OutputCompleted") }), z.object({ type: z.literal("Warning"), message: z.string() }), z.object({ type: z.literal("Error"), error: harnessErrorSchema }), z.object({ type: z.literal("ExecutionCompleted") }), z.object({ type: z.literal("ExecutionCancelled") }), z.object({ type: z.literal("SessionCreated"), sessionId: id }), z.object({ type: z.literal("SessionClosed"), sessionId: id }), z.object({ type: z.literal("ToolRequested"), toolCallId: z.string(), toolId: z.string() }), z.object({ type: z.literal("ToolStarted"), toolCallId: z.string(), toolId: z.string() }), z.object({ type: z.literal("ToolCompleted"), toolCallId: z.string(), toolId: z.string(), result: modelToolResultSchema }), z.object({ type: z.literal("ApprovalRequested"), approvalId: z.string(), toolCallId: z.string(), toolId: z.string(), input: jsonValueSchema, descriptor: toolDescriptorSchema.optional(), executionId: z.string(), sessionId: z.string(), traceId: z.string() }), z.object({ type: z.literal("ApprovalResolved"), approvalId: z.string(), decision: z.enum(["approve", "deny"]), executionId: z.string(), sessionId: z.string(), traceId: z.string(), reason: z.string().optional() })]);
