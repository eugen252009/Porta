...
export interface DelegationRequest { objective: string; constraints?: readonly string[]; acceptanceCriteria?: readonly string[]; model?: string; restrict?: readonly DelegationPermission[]; budget?: { maxTurns?: number; maxToolCalls?: number; maxTokens?: number }; scratchpad?: ScratchpadPrefill }
...
export interface DelegatedTaskMetadata { readonly delegationId: string; readonly parentNodeId: string; readonly parentIdentity: string; readonly childNodeId: string; readonly authority: "child" | "parent-projection" }
...
export class DelegatedTaskStore {
  ...
  async create(parentSessionId: string, parentPermissions: readonly DelegationPermission[], request: DelegationRequest, depth = 0): Promise<DelegatedTask> {
    ...
    const task: DelegatedTask = { id, parentSessionId, objective: request.objective, ...(request.constraints ? { constraints: [...request.constraints] } : {}), ...(request.acceptanceCriteria ? { acceptanceCriteria: [...request.acceptanceCriteria] } : {}), ...(request.restrict ? { restrictions: [...request.restrict] } : {}), ...(request.model ? { requestedModel: request.model } : {}), ...(request.budget ? { budget: request.budget } : {}), effectivePermissions: effectivePermissions(parentPermissions, request.restrict), scratchpadNamespace: `delegation-${id}`, status: "pending", attempts: [], depth, version: 1, authority: "local", createdAt: now, updatedAt: now };
  ...
  async createChild(parentSessionId: string, parentPermissions: readonly DelegationPermission[], request: DelegationRequest, metadata: DelegatedTaskMetadata): Promise<DelegatedTask> { const existing = this.findByDelegation(metadata.delegationId, metadata.parentIdentity); if (existing) return existing; const task = await this.create(parentSessionId, [...parentPermissions, "agent.delegate"], request); const updated: DelegatedTask = { ...task, effectivePermissions: task.effectivePermissions.filter((permission) => permission !== "agent.delegate"), ...metadata, authority: "child", version: task.version + 1, updatedAt: new Date().toISOString() }; this.tasks.set(updated.id, updated); await this.persistence?.save(updated); return cloneTask(updated); }
  async recordProjection(parentSessionId: string, request: DelegationRequest, accepted: { readonly delegationId: string; readonly childNodeId: string; readonly childTaskId: string; readonly taskVersion: number; readonly state: DelegatedTaskStatus }, parentIdentity: string): Promise<DelegatedTask> { const existing = this.findByDelegation(accepted.delegationId, parentIdentity); if (existing) return existing; const now = new Date().toISOString(); const task: DelegatedTask = { id: accepted.delegationId, parentSessionId, objective: request.objective, ...(request.constraints ? { constraints: [...request.constraints] } : {}), ...(request.acceptanceCriteria ? { acceptanceCriteria: [...request.acceptanceCriteria] } : {}), ...(request.restrict ? { restrictions: [...request.restrict] } : {}), ...(request.model ? { requestedModel: request.model } : {}), ...(request.budget ? { budget: request.budget } : {}), effectivePermissions: [], scratchpadNamespace: `remote-${accepted.delegationId}`, status: accepted.state, attempts: [], depth: 0, version: accepted.taskVersion, createdAt: now, updatedAt: now, delegationId: accepted.delegationId, parentNodeId: parentIdentity, parentIdentity, childNodeId: accepted.childNodeId, authority: "parent-projection", lastSeenAt: now, stale: false }; this.tasks.set(task.id, task); await this.persistence?.save(task); return cloneTask(task); }
