...
export class DevelopmentRunner {
  ...
  async release(sessionId: string): Promise<Task> {
    if (!this.releaseCapabilities) { const task = await this.requireTask(sessionId); const state = this.requireState(task); const message = "Release capabilities or image repository configuration is unavailable."; return this.tasks.update(sessionId, task.id, task.version, { type: "set_development", development: { ...state, attention: { reason: "user_input_required", message }, pendingIntervention: { action: "provide_input", message, requestedAt: this.now() }, lastEvent: message, updatedAt: this.now() } }); }
    ...
    try {
      ...
      task = await this.releaseStep(task, "committing", "git commit", "git", async () => { const result = await this.releaseCapabilities!.commit({ message: `porta: ${state.goal}`, ...(baseRevision ? { expectedRevision: baseRevision } : {}) }); return { phase: "pushing", patch: { commitSha: result.revision, originalRevision: baseRevision ?? result.revision, ...(result.branch ? { branch: result.branch } : {}), commitMessage: result.subject } }; });
      const commitSha = this.requireState(task).commitSha!;
      task = await this.releaseStep(task, "pushing", "git push", "git", async () => { const result = await this.releaseCapabilities!.push({ revision: commitSha }); if (result.revision !== commitSha) throw failure("CAPABILITY_CONFLICT", "Push revision does not match the committed revision."); return { phase: "building_image", patch: { push: { revision: result.revision, ...(result.remote ? { remote: result.remote } : {}), ...(result.branch ? { branch: result.branch } : {}) } } }; });
      task = await this.releaseStep(task, "building_image", "docker build", "build", async () => { const result = await this.releaseCapabilities!.build({ revision: commitSha, tag: commitSha.slice(0, 12) }); return { phase: "pushing_image", patch: { image: { reference: result.reference, tag: result.tag, ...(result.digest ? { digest: result.digest } : {}) } } }; });
      const image = this.requireState(task).image!;
      task = await this.releaseStep(task, "pushing_image", "docker push", "image_push", async () => { const result = await this.releaseCapabilities!.pushImage({ reference: image.reference, sourceRevision: commitSha, repository: image.reference.slice(0, image.reference.lastIndexOf(":")), tag: image.tag ?? commitSha.slice(0, 12), ...(image.digest ? { digest: image.digest } : {}) }); return { phase: "deploying", patch: { image: { reference: result.reference, tag: result.tag, ...(result.digest ? { digest: result.digest } : {}), ...(result.latestReference ? { latestReference: result.latestReference } : {}) } } }; });
      ...
      let handoff: Awaited<ReturnType<DevelopmentReleaseCapabilities["deploy"]>>;
      try { handoff = await this.releaseCapabilities!.deploy({ deploymentId, sourceRevision: commitSha, imageReference: latest.image!.reference, ...(latest.image?.digest ? { imageDigest: latest.image.digest } : {}), ...(prepared.previousCommit ? { previousRevision: prepared.previousCommit } : {}), ...(prepared.previousImageReference ? { previousImageReference: prepared.previousImageReference } : {}) }); } catch (error) { return this.releaseFailure(task, error instanceof Error ? error.message : "Deployment trigger state is unknown."); }
  ...
  private async releaseStep(task: Task, phase: DevelopmentPhase, command: string, category: DevelopmentExecutionCategory, action: () => Promise<{ readonly phase: DevelopmentPhase; readonly patch?: Partial<DevelopmentState> }>): Promise<Task> {
    ...
    const running: DevelopmentState = { ...state, phase, executionRecords: [...(state.executionRecords ?? []), { id: recordId, category, command, startedAt, status: "running" }], execution: { phase, status: "running", attempt: (state.execution?.attempt ?? 0) + 1, startedAt }, lastEvent: `Started ${phase}.`, updatedAt: startedAt };
