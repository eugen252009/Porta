(() => {
  ...
  function findWritingBlockArtifact(header) {
    const regionSelector = '[data-writing-block-fullscreen-editor-region="true"]';
    let ancestor = header.parentElement;
    for (let depth = 0; ancestor && depth < 8; depth++, ancestor = ancestor.parentElement) { const regions = []; if (ancestor.matches?.(regionSelector)) regions.push(ancestor); regions.push(...ancestor.querySelectorAll(regionSelector)); const unique = [...new Set(regions)].filter((region) => !header.contains(region) && !extensionOwned(region)); if (unique.length === 1) return { type: "writing-block", root: ancestor, header, source: unique[0] }; if (unique.length > 1) return null; if (isAssistantCandidate(ancestor)) break; }
    return null;
  }
  ...
  function inlineMarkdown(node) { if (node.nodeType === Node.TEXT_NODE) return node.nodeValue || ""; if (node.nodeType !== Node.ELEMENT_NODE || extensionOwned(node)) return ""; const tag = node.tagName.toLowerCase(); if (tag === "br") return "\n"; if (tag === "strong" || tag === "b") return `**${[...node.childNodes].map(inlineMarkdown).join("")}**`; if (tag === "em" || tag === "i") return `*${[...node.childNodes].map(inlineMarkdown).join("")}*`; if (tag === "code") return `\`${node.textContent || ""}\``; if (tag === "a" && node.getAttribute("href")) return `[${[...node.childNodes].map(inlineMarkdown).join("")}](${node.getAttribute("href")})`; return [...node.childNodes].map(inlineMarkdown).join(""); }
  function blockMarkdown(node) { if (node.nodeType === Node.TEXT_NODE) return node.nodeValue || ""; if (node.nodeType !== Node.ELEMENT_NODE || extensionOwned(node)) return ""; const tag = node.tagName.toLowerCase(); if (tag === "pre") { const code = node.querySelector("code"); return "\n\n```\n" + ((code ? code.textContent : node.textContent) || "") + "\n```"; } if (/^h[1-6]$/.test(tag)) return "\n\n" + "#".repeat(Number(tag.slice(1))) + " " + [...node.childNodes].map(inlineMarkdown).join(""); if (tag === "p") return "\n\n" + [...node.childNodes].map(inlineMarkdown).join(""); if (tag === "blockquote") return "\n\n" + [...node.childNodes].map(blockMarkdown).join("").split("\n").filter((line, index, lines) => line || lines[index - 1]).map((line) => line ? "> " + line : ">").join("\n"); if (tag === "ul" || tag === "ol") { let index = 0; return "\n\n" + [...node.children].filter((child) => child.tagName.toLowerCase() === "li").map((child) => (tag === "ol" ? (++index) + "." : "-") + " " + [...child.childNodes].map(inlineMarkdown).join("")).join("\n"); } if (tag === "hr") return "\n\n---"; return [...node.childNodes].map(blockMarkdown).join(""); }
  function serializeWritingBlock(source) { return [...source.childNodes].map(blockMarkdown).join("").replace(/\r\n?/g, "\n").replace(/\n{3,}/g, "\n\n").trim(); }
  function extractArtifactMarkdown(artifact) { if (artifact.type === "writing-block") return serializeWritingBlock(artifact.source); const code = artifact.source.matches("pre") ? artifact.source.querySelector("code") : artifact.source.querySelector("pre code, code"); return (code ? code.textContent : artifact.source.textContent)?.trim() || ""; }
  ...
  async function submitArtifact(container, artifact, node, newSession) {
    ...
    try { const content = extractArtifactMarkdown(artifact); if (!content) throw new Error("No standalone artifact content found."); const key = newSession ? crypto.randomUUID() : artifact.root.dataset.portaIdempotency || (artifact.root.dataset.portaIdempotency = crypto.randomUUID()); const accepted = await window.PortaExtensionClient.submit(content, node.id, key, conversationIdentity(), newSession); setStatus(container, accepted.jobId ? `✓ Accepted by Porta · job ${accepted.jobId.slice(0, 8)}${accepted.durable ? " · saved; you can close the browser" : " · memory only; restart will lose this job"}` : `✓ Accepted by ${accepted.nodeId} · remote execution status not tracked here`); if (accepted.statusUrl) { const link = document.createElement("a"); link.href = accepted.statusUrl; link.target = "_blank"; link.rel = "noopener noreferrer"; link.textContent = " Open Porta"; container.append(link); } } catch (error) { setStatus(container, error instanceof Error ? error.message : "Porta submission failed."); }
  }
