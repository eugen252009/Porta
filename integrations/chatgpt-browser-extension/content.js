(() => {
  ...
  const processedArtifacts = new WeakSet();
  const candidateTimers = new WeakMap();
  const style = document.createElement("style");
  style.textContent = `.porta-extension-control{position:relative;display:inline-flex;z-index:20;margin:8px 0;font:12px system-ui;color:#d7f7f1}.porta-extension-control button{border:1px solid #5d7775;border-radius:6px;padding:5px 8px;color:inherit;background:#182321;cursor:pointer}.porta-extension-control button{text-align:left}.porta-extension-menu{position:absolute;top:32px;right:0;display:grid;min-width:210px;padding:5px;border:1px solid #5d7775;border-radius:7px;background:#182321;box-shadow:0 8px 25px #0008}.porta-extension-menu button{text-align:left;border:0;margin:1px;padding:7px;background:transparent}.porta-extension-menu button:hover{background:#263a36}.porta-extension-menu button:disabled{opacity:.5;cursor:default}.porta-extension-status{padding:6px 8px;color:#9db2ad;white-space:nowrap}`;
  ...
  function assistantRoot(node) {
    const element = node instanceof Element ? node : node.parentElement;
    if (!element) return undefined;
    if (isAssistantCandidate(element)) return element;
    const closest = element.closest(ASSISTANT_SELECTORS);
    return closest && isAssistantCandidate(closest) ? closest : undefined;
  }
  function extensionOwned(node) {
    const element = node instanceof Element ? node : node.parentElement;
    return Boolean(element?.closest(`[${MARKER}]`));
  }
  ...
  function processCandidate(candidate) {
    for (const artifact of candidate.matches(ARTIFACT_SELECTORS) ? [candidate, ...candidate.querySelectorAll(ARTIFACT_SELECTORS)] : [...candidate.querySelectorAll(ARTIFACT_SELECTORS)]) { if (processedArtifacts.has(artifact) || !text(artifact)) continue; processedArtifacts.add(artifact); inject(artifact); }
  }
  function queueCandidate(candidate) {
    if (candidateTimers.has(candidate)) clearTimeout(candidateTimers.get(candidate));
    candidateTimers.set(candidate, setTimeout(() => { candidateTimers.delete(candidate); processCandidate(candidate); }, 650));
  }
  function queueFromAddedNode(node) {
    if (!(node instanceof Element) || extensionOwned(node)) return;
    const candidate = assistantRoot(node); if (candidate) queueCandidate(candidate);
    for (const nested of node.querySelectorAll(ASSISTANT_SELECTORS)) if (isAssistantCandidate(nested)) queueCandidate(nested);
  }
  function initialDiscovery() { for (const candidate of document.querySelectorAll(ASSISTANT_SELECTORS)) if (isAssistantCandidate(candidate)) processCandidate(candidate); }
