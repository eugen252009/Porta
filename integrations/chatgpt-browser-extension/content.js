(() => {
  ...
  function inject(copyButton, artifact) {
    ...
    const container = document.createElement("span"); container.className = "porta-extension-control"; container.setAttribute(MARKER, "true"); const button = document.createElement("button"); button.type = "button"; button.textContent = "Send to Porta ▾"; button.onclick = () => { container.querySelector(".porta-extension-menu")?.remove(); void showTargets(container, artifact); }; container.append(button); actionContainer.append(container);
  }
