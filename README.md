...
Git is opt-in through `git.enabled` and exposes session-scoped `git/status`, `git/credential_status`, `git/branches`, `git/diff`, `git/show`, `git/log`, `git/fetch`, `git/commit`, and `git/push` tools. Settings can create and rotate Porta-generated GitHub Ed25519 SSH keys, display only their public key/fingerprint, and verify access against trusted host keys; generated credentials can be explicitly assigned for clone/fetch/push. The CLI backend uses direct argv, isolated Git configuration, bounded output, and the session workspace as its repository root. Fetch, commit, and push remain subject to the configured authorization policy; none happen implicitly. See [Session workspaces and Git](docs/SESSION-WORKSPACES.md) for clone, credential, save/delete, and security behavior.
...
The web server can load HomeAuth's v1 admission verifier as a real Porta plugin. Configure both `HOMEAUTH_PUBLIC_KEY` (path to the authority's raw Ed25519 public key, base64url or base64) and `HOMEAUTH_SERVICE_ID` (the allocated decimal service ID) to enable it. It verifies the signed token, its time bounds and this service's grant bit. Human subjects become namespaced human principals; service/legacy subjects become namespaced integrations with no Porta permissions (never nodes). Admission only proves identity and service access, not roles. With neither variable set, behavior is unchanged; setting only one is a startup error.

Preferred transport is the dedicated `HomeAuth-Admission` header. A HomeAuth route can authenticate the browser session/JWT and use the `admission` adapter to issue or preserve a grant-checked token. Alternatively, the sidecar can use `HOMEAUTH_FORWARD_ADMISSION=true`: it first verifies the incoming bearer token and local service grant, applies its configured adapter, strips synthesized identity headers and cookies, then forwards only the signed token in `HomeAuth-Admission`. Porta independently verifies it. Direct callers cannot supply trusted `X-HomeAuth-*`/`Remote-User` headers, and the Authority route rejects caller-supplied `HomeAuth-Admission`; the sidecar overwrites that header after verification. Do not expose Porta's upstream to untrusted networks.

The API-only listener supports `PORTA_LLM_AUTH_MODE=compatible` (default) and `required`. Compatible mode retains `PORTA_LLM_API_SECRET` for legacy clients; when the plugin is active, a HomeAuth admission token is still independently verified, including if accompanied by the compatibility bearer. Required mode rejects static-secret-only requests and accepts verified HomeAuth admission. During migration, keep the secret mounted while forwarding signed admission; then switch the sidecar from `backend-bearer` to `native` plus forwarding and qualify it. Once every API client uses an admission credential, the static secret is unnecessary for this listener and can be removed in a separate approved change. HomeAuth remains the identity/session/token authority; Porta only verifies the signed grant and maps a least-privilege principal. Porta's local WebAuthn login remains available unless frontdoor configuration selects HomeAuth instead.
