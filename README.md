...
Optional filesystem tools can be enabled with `filesystem.root`. Each Porta conversation session receives a separate server-owned workspace directory; filesystem, command, and Git providers are bound to that directory rather than sharing the configured root. New workspaces are empty unless the user selects a saved project or asks Porta to clone a Git repository. `filesystem.root` remains required to enable filesystem tools and configures the existing base workspace/persistence context; it is not copied into each session. Mutation remains disabled unless `filesystem.mutation.enabled` is explicitly set; when enabled, `filesystem/write_file` and `filesystem/patch_file` use atomic, hash-checked updates. Mutation limits default to 2 MiB writes and 8 MiB patch targets. File summaries use a generic content-reduction boundary. Agent-authored notes are provided separately by session-scoped `scratchpad/write`, `scratchpad/append`, `scratchpad/read`, and `scratchpad/list`; scratchpad contents remain off-context until explicitly read and are not persisted across restarts.
...
Controlled command execution is opt-in through `execution.enabled` and requires a configured filesystem root. Commands use direct argv execution, an explicit allowlist, bounded stdout/stderr, a session-workspace cwd, and the existing authorization path. Sandbox selection prefers an available Linux Bubblewrap backend, then safely falls back only when the configured policy permits it. Workspace authorization does not register command execution unless the selected sandbox enforces its filesystem boundary; host-process sandboxing is reported honestly as best-effort/unsupported rather than being presented as isolation. Bubblewrap is optional; it is not required for read-only Porta startup.
...
Git is opt-in through `git.enabled` and exposes session-scoped `git/status`, `git/branches`, `git/diff`, `git/show`, `git/log`, `git/fetch`, `git/commit`, and `git/push` tools. The CLI backend uses direct argv, isolated Git configuration, bounded output, and the session workspace as its repository root. Fetch, commit, and push remain subject to the configured authorization policy; none happen implicitly. See [Session workspaces and Git](docs/SESSION-WORKSPACES.md) for clone, credential, save/delete, and security behavior.

Durable conversation/task/artifact state is opt-in through `persistence.enabled`. Session workspace ownership and the server-side Git credential vault are persisted separately under `PORTA_DATA_DIR` (default `.porta`) even when SQLite persistence is disabled. SQLite stores conversation turns, scratchpad entries, task/evidence state, and immutable artifact payloads under the configured workspace (for example `.porta/porta.db`). Use `PORTA_SESSION=<id>` or `--session <id>` with the terminal to resume an existing open session. SQLite persistence uses plaintext local storage and requires a Node runtime with `node:sqlite`; memory stores remain the default.
...
Set `authorization.mode: "workspace"` to allow ordinary reads, summaries, listings, searches, and enabled mutations inside `filesystem.root` without repeated approval. Sensitive paths (including `.env*`, credentials, private keys, and runtime metadata) require human approval; search/listings omit them. Aliases and outside-root access are denied, and commands, Git, and other tools remain approval-gated even if listed in `jobs.unattendedTools`. This is a tool policy, not OS isolation or secret-content detection. See [workspace permissions](docs/WORKSPACE-PERMISSIONS.md) for defaults, additional protected paths, and limits. The default `require-approval` mode is unchanged.

### Browser-independent jobs

Local extension submissions and Web composer messages now enter a server-owned FIFO job queue. With SQLite enabled, acceptance persists the prompt and receipt before execution. The browser can close; job activity, failures, command result previews, and completion remain queryable later. Queued jobs resume after restart; previously started jobs are marked interrupted and are not replayed automatically. Unattended canonical tool permissions are explicitly opt-in through `jobs.unattendedTools`; safe approval defaults remain unchanged. Completion is execution completion, not automatic certification of a natural-language task.

See [browser-independent jobs](docs/BROWSER-INDEPENDENT-JOBS.md) for configuration, limits, security boundaries, API behavior, and qualification. `porta.docker.jobs.json` is an opt-in development example; it does not replace a running node's configuration automatically.

### Local browser application

The browser workspace uses a session sidebar and a focused conversation canvas with a persistent composer. On opening, Porta resumes the last open session or creates one automatically on Local development with the configured currently available model. The sidebar lets you switch chats. Start a new chat through its pre-session workspace/provider/model chooser; the active chat's workspace and model are shown as fixed context rather than silently switching underneath it. Node selection, session status, tool activity, and approval controls use the existing Web APIs; settings and passkey authentication remain separate. Conversation history refreshes from persisted snapshots without replacing an in-flight response. The responsive UI uses local assets only, with no external fonts or frontend service dependencies.
...
#### Optional HomeAuth admission plugin

The API-only listener can load HomeAuth's v1 admission verifier as a real Porta plugin. Configure both `HOMEAUTH_PUBLIC_KEY` (path to the authority's raw Ed25519 public key, base64url or base64) and `HOMEAUTH_SERVICE_ID` (the allocated decimal service ID) to enable it. It verifies the signed token, its time bounds and this service's grant bit. Human subjects become namespaced human principals; service/legacy subjects become namespaced integrations with no Porta permissions (never nodes). Admission only proves identity and service access, not roles. With neither variable set, behavior is unchanged; setting only one is a startup error.

HomeAuth admission is scoped to the API-only listener's `/v1` machine API. The browser listener and its `/api/*` routes continue to use Porta's own WebAuthn, browser session, and Porta login credentials; a HomeAuth admission never signs a user into the browser UI or replaces that session. Browser requests still reject ambiguous duplicate credentials and spoofed `X-HomeAuth-*`/`Remote-User` identity headers. Preferred transport for the machine API is the dedicated `HomeAuth-Admission` header. A HomeAuth frontdoor can authenticate the browser session/JWT and use the `admission` adapter to issue or preserve a grant-checked token for that API. Alternatively, the sidecar can use `HOMEAUTH_FORWARD_ADMISSION=true`: it first verifies the incoming bearer token and local service grant, applies its configured adapter, strips synthesized identity headers and cookies, then forwards only the signed token in `HomeAuth-Admission`. Porta independently verifies it. The Authority route rejects caller-supplied `HomeAuth-Admission`; the sidecar overwrites that header after verification. Do not expose Porta's upstream to untrusted networks.

The API-only listener supports `PORTA_LLM_AUTH_MODE=compatible` (default) and `required`. Compatible mode retains `PORTA_LLM_API_SECRET` for legacy `/v1` clients; when the plugin is active, a HomeAuth admission token is still independently verified, including if accompanied by the compatibility bearer. Required mode rejects static-secret-only requests and accepts verified HomeAuth admission. During migration, keep the secret mounted while forwarding signed admission; then switch the sidecar from `backend-bearer` to `native` plus forwarding and qualify it. Once every API client uses an admission credential, the static secret is unnecessary for this listener and can be removed in a separate approved change. HomeAuth remains the identity/session/token authority; Porta only verifies the signed grant. Porta's local WebAuthn/browser-session flow is independent of API admission.
