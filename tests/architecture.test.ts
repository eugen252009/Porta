...
describe("architecture boundaries", () => {
  ...
  it("keeps SQLite knowledge in persistence adapters", () => { for (const file of ["src/conversation.ts", "src/scratchpad.ts", "src/task.ts", "src/agent.ts"]) expect(readFileSync(join(process.cwd(), file), "utf8")).not.toMatch(/sqlite|DatabaseSync|persistence-sqlite/i); });
});
