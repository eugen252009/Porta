...
describe("architecture boundaries", () => {
  ...
  it("keeps Ollama imports confined to its adapter", () => {
    ...
    for (const file of sourceFiles.filter((file) => !file.includes("model-ollama") && !file.includes("model-picker") && !file.includes("porta-application") && !file.includes("porta-config") && !file.endsWith("src/index.ts"))) expect(readFileSync(file, "utf8")).not.toMatch(/ollama/i);
  });
  ...
  it("keeps Git behind its backend and away from task/filesystem ownership", () => { const source = readFileSync(join(process.cwd(), "src/git.ts"), "utf8"); expect(source).not.toMatch(/TaskStore|FilesystemMutationEngine|ExecutionToolProvider|ToolRouter/); });
  it("keeps SQLite knowledge in persistence adapters", () => { for (const file of ["src/conversation.ts", "src/scratchpad.ts", "src/task.ts", "src/agent.ts", "src/artifact.ts"]) expect(readFileSync(join(process.cwd(), file), "utf8")).not.toMatch(/sqlite|DatabaseSync|persistence-sqlite/i); });
  it("keeps artifact storage independent from task and scratchpad ownership", () => { const source = readFileSync(join(process.cwd(), "src/artifact.ts"), "utf8"); expect(source).not.toMatch(/TaskStore|ScratchpadStore|ConversationStore|Ollama|MCP/); });
});
