...
server.registerTool("echo", { description: "Returns input", inputSchema: { value: z.unknown() } }, async ({ value }) => { if (process.env.MCP_CALL_COUNT_FILE) appendFileSync(process.env.MCP_CALL_COUNT_FILE, "echo\n"); return { content: [{ type: "text", text: JSON.stringify(value) }], structuredContent: { value } }; });
