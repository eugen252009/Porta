...
describe("ChatGPT Porta extension", () => {
  ...
  it("extracts assistant artifacts without privileged Porta operations", () => { expect(content).toContain("data-message-author-role=\"assistant\""); expect(content).toContain("renderedToMarkdown"); expect(client).toContain("chrome.runtime.sendMessage"); expect(content).not.toMatch(/ssh|docker|filesystem|execution\.run/i); });
  it("uses a scoped extension origin and no embedded credential", () => { expect(manifest.manifest_version).toBe(3); expect(manifest.optional_host_permissions).toEqual(expect.arrayContaining(["https://*/*"])); expect(manifest.background.service_worker).toBe("background.js"); expect(background).toContain("Authorization"); expect(client).not.toContain("Authorization"); expect(client).not.toContain("chrome.storage"); expect(background).toContain("UNKNOWN_EXTENSION_ACTION"); expect(background).not.toContain("message.url"); });
});
