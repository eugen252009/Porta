...
describe("ChatGPT Porta extension", () => {
  it("uses explicit user action and idempotent artifact controls", () => { expect(content).toContain("Send to Porta ▾"); expect(content).toContain("MutationObserver"); expect(content).toContain("data-porta-control"); expect(content).toContain("crypto.randomUUID()"); expect(content).toContain("data-testid=code-block"); expect(content).toContain("processedArtifacts"); expect(content).not.toContain("setInterval"); });
  it("extracts assistant artifacts without privileged Porta operations", () => { expect(content).toContain("data-message-author-role=\"assistant\""); expect(content).toContain("extractArtifactMarkdown"); expect(content).toContain("artifact.matches(\"pre\")"); expect(content).toContain("code.textContent"); expect(content).toContain("queueFromAddedNode"); expect(content).not.toContain("innerText"); expect(content).not.toContain("querySelector(\".markdown, .prose\")"); expect(client).toContain("chrome.runtime.sendMessage"); expect(content).not.toMatch(/ssh|docker|filesystem|execution\.run/i); });
  it("coalesces streaming mutations and ignores extension-owned DOM", () => { expect(content).toContain("candidateTimers"); expect(content).toContain("setTimeout(() =>"); expect(content).toContain("extensionOwned(node)"); expect(content).toContain("addedNodes"); expect(content).not.toContain("scan()"); });
